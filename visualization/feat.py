"""
use COLORMAP_JET ColorMap to visual the activation of features.
"""
...
def show_cam_on_image_save(mask, num, laterals):
    root = os.path.join('/home/sensetime/Desktop/result/crop_in_fpn_1.0/person', '800_1_4', str(laterals))
...
def show_mask(mask, laterals):
    """
    use COLORMAP_JET ColorMap to visual the activation of features.
    Args:
        mask:<tensor>(N, C, H, W)
        laterals: <tensor>(1, ) the stage of feature
    """
    N, C, H, W = mask.shape
    for i in range(N):
        mask_tmp = mask[i]
        for num, j in enumerate(mask_tmp):
            show_cam_on_image_save(j, num, laterals)